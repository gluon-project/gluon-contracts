...
contract ERC223TokenFactory {
    ...
    function ERC223TokenFactory() {
      //upon creation of the factory, deploy a ERC223Token (parameters are meaningless) and store the bytecode provably.
      address verifiedToken = createERC223Token(10000, "Verify Token", 3, "VTX");
    ...
    function verifyERC223Token(address _tokenContract) constant returns (bool) {
    ...
    function createERC223Token(uint256 _initialAmount, string _name, uint8 _decimals, string _symbol) returns (address) {

        ERC223Token newToken = (new ERC223Token(_initialAmount, _name, _decimals, _symbol));
